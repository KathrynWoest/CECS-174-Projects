...
def char_num(string):  # C - Alexis Lopez-Ortiz
    ...
    print(f'Your text contains {len(alpha_string)} alphabetic characters, of which {num_e} ({percent_e:.1f}%) are "e".')
...
def chr_num2(string, letter):  # D - Alexis Lopez-Ortiz
    ...
    print(f"Your text contains {len(alpha_string)} alphabetic characters, of which {num_letter}"
...
def no_e_words(quote):  # G - Kathryn Woest
    ...
    print(f"The percent of words that have no 'e' in them is {percent:.01f}%.")
    print(new_quote)
...
def avoids_user_forbidden(quote, forbidden):  # H2 - Kathryn Woest
    ...
    print(f"The number of words that don't contain any of the characters in {forbidden} is {no_forbidden}.")
...
def quote_uses_all(quote, use_chr):  # J2 - Kathryn Woest
    ...
    print(f"The number of words that use all of the characters in {use_chr} at least once is {use_all_count}.")
...
def quote_is_abecedarian(quote):  # K2 - Kathryn Woest
    ...
    while word_location <= length:
        ...
        if alphabetical:
            is_alpha += 1
        word_location = new_word_loc + 1
    print(f"The number of words that are in alphabetical order is {is_alpha}.")
...
def remove_duplicates(og_string):  # Q - Kathryn Woest
    ...
    char = 0
    start = 0
    new_string = ""  # starts the new string off with nothing in it
    while char < length:
        dupl = og_string.count(og_string[char])
        if dupl != 1:  # if the character being tested has duplicates
            new_string += og_string[start:char + 1]  # add to the new string all the characters from start to current
            start = char + 1  # make the new starting location where we are testing plus one (to not include the dupl)
            og_string = og_string.replace(og_string[char], "")  # delete all instances of the dupl from the og string
            length = len(og_string)  # recalculate the length of the new og string for the while loop condition
        new_string += og_string[char]  # if not a dupl, just add the character to the new string
        start += 1  # increment start by one so the character is not added again
        char += 1  # increment the index of the string that is being tested
    print(new_string)


def main():  # Kathryn Woest and Alexis Lopez-Ortiz
    # excerpt from the poem "Sheep Song", by Japanese poet Nakahara Chuuya (translated by Christian Nagle)
    sheep_song = '''O expectations, stale and dismal airs, leave this body of mine!\n\
...
I will endure my solitude, arms seeming already useless. O eyes that open doubtfully,\n\
open eyes that stay motionless for a while, ah, heart, that believes in others more than itself,\n\
O expectations, stale and dismal airs, leave, leave this body of mine!\n\
I enjoy nothing anymore but my wretched dreams.\n'''

    print("This project uses a quote for several of the functions to display how strings can be manipulated.\n"
          "The quote is an excerpt from the poem 'Sheep Song',"
          " written by Japanese poet Nakahara Chuuya and translated by Christian Nagle.\n")

    # A
    print("Part A:\nThe original quote is printed, followed by the quote being mirrored.")
    print(mirror(sheep_song), "\n")

    # B
    print("Part B:\nThe function will remove a letter of your choice from the quote.")
    user_letter = input("What letter should be removed? ")
    print(remove(sheep_song, user_letter))
    ...
    chr_num2(sheep_song, user_character)
    print()
    ...
    avoids_user_forbidden(sheep_song, user_forbidden_h)
    print()
    ...
    quote_uses_all(sheep_song, user_required_j)
    ...
    quote_is_abecedarian(sheep_song)
    print()
    ...
    print(find(sheep_song, user_character_l))
    print()
    ...
    print(find2(sheep_song, user_character_m, user_start_m))
    print()
